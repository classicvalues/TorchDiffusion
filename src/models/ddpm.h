...
public:
    ...
    DDPMImpl(const std::shared_ptr<Module> &model, std::tuple<int, int> &img_size, int T = 1000,
    ...
    torch::Tensor ddim_sample(std::shared_ptr<std::string> path = nullptr, int n = 4,
                              std::shared_ptr<torch::Tensor> z_samples = nullptr,
